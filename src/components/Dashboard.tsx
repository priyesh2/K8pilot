...
import { Activity, Database, Server, Cpu, Filter, AlertCircle, Maximize2, RefreshCw, TrendingUp, Zap, Shield, BarChart3 } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ currentNS, namespaces, onNamespaceChange }) => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '24px' }}>
        ...
        <section>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{pods.length} total</span>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: 'calc(100vh - 400px)', overflowY: 'auto' }}>
            {loading && pods.length === 0 ? (
              ...
              <div key={pod.id} className="glass-card fade-in" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px' }}>
                ...
                <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexShrink: 0 }}>
                  ...
                  <button onClick={() => handleRestart(pod)} title="Restart" style={{ padding: '6px', background: 'rgba(255,255,255,0.04)', border: 'none', color: 'var(--text-secondary)', borderRadius: '6px', cursor: 'pointer', transition: 'all 0.15s' }}
                    onMouseEnter={e => { (e.target as HTMLElement).style.color = 'var(--warning)'; (e.target as HTMLElement).style.background = 'rgba(245,158,11,0.1)'; }}
                    onMouseLeave={e => { (e.target as HTMLElement).style.color = 'var(--text-secondary)'; (e.target as HTMLElement).style.background = 'rgba(255,255,255,0.04)'; }}
                  >
                    <RefreshCw size={14} />
                  </button>
                  <button onClick={() => handleScale(pod)} title="Scale" style={{ padding: '6px', background: 'rgba(255,255,255,0.04)', border: 'none', color: 'var(--text-secondary)', borderRadius: '6px', cursor: 'pointer', transition: 'all 0.15s' }}
                    onMouseEnter={e => { (e.target as HTMLElement).style.color = 'var(--accent-blue)'; (e.target as HTMLElement).style.background = 'rgba(99,102,241,0.1)'; }}
                    onMouseLeave={e => { (e.target as HTMLElement).style.color = 'var(--text-secondary)'; (e.target as HTMLElement).style.background = 'rgba(255,255,255,0.04)'; }}
                  >
                    <Maximize2 size={14} />
                  </button>
          ...
          <div>
            ...
            <div className="glass-card" style={{ padding: '16px' }}>
              {topRestarters.filter(p => p.restarts > 0).length === 0 ? (
                ...
                <div key={pod.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < topRestarters.filter(p => p.restarts > 0).length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
                  <div>
